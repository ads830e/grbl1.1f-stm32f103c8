...
void Stepper_Driver_Interrupt(void)   //TIM3中断:The Stepper Driver Interrupt
{
  ...
  if(bit_istrue(st.step_outbits,bit(X_STEP_BIT))!=bit_istrue(step_port_invert_mask,bit(X_STEP_BIT))){
    if(bit_istrue(st.dir_outbits,bit(X_DIRECTION_BIT))) Xstepper(1);
    else Xstepper(-1);
  }
  if(bit_istrue(st.step_outbits,bit(Y_STEP_BIT))!=bit_istrue(step_port_invert_mask,bit(Y_STEP_BIT))){
    if(bit_istrue(st.dir_outbits,bit(Y_DIRECTION_BIT))) Ystepper(1);
    else Ystepper(-1);
  }

  // Enable step pulse reset timer so that The Stepper Port Reset Interrupt can reset the signal after
  // exactly settings.pulse_microseconds microseconds, independent of the main Timer1 prescaler.
  HW_TIM_PortResetInterrupt_ValueConfig(1,st.step_pulse_time); 	//设置定时器重装值
  ...
  if (st.exec_segment == NULL)
  {
    ...
    if (segment_buffer_head != segment_buffer_tail)
    {
      ...
      #ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
        HW_TIM_DriverInterrupt_ValueConfig(1,st.exec_segment->cycles_per_tick);
        //使能AMASS算法就使用固定值分频【分频值需要调整】
      #else
        // With AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).
        if(st.exec_segment->prescaler<=1) HW_TIM_DriverInterrupt_ValueConfig(5,st.exec_segment->cycles_per_tick/2);
        else if(st.exec_segment->prescaler==2) HW_TIM_DriverInterrupt_ValueConfig(20,st.exec_segment->cycles_per_tick);
        else HW_TIM_DriverInterrupt_ValueConfig(160,st.exec_segment->cycles_per_tick);
        
        //不使能AMASS算法就使用硬件分支产生分频
      #endif
...
void Stepper_Reset_Interrupt(void)   //TIM4中断:The Stepper Port Reset Interrupt
{
  ...
  {
    // Begin step pulse.
    HW_GPIO_Write(STEP_GPIO,X_STEP_GPIO_PIN,bit_istrue(st.step_bits,bit(X_STEP_BIT)));
    HW_GPIO_Write(STEP_GPIO,Y_STEP_GPIO_PIN,bit_istrue(st.step_bits,bit(Y_STEP_BIT)));
    HW_GPIO_Write(STEP_GPIO,Z_STEP_GPIO_PIN,bit_istrue(st.step_bits,bit(Z_STEP_BIT)));
  }
#endif
...
void stepper_init()
{
  // Configure step and direction interface pins
    //初始化步进和方向接口引脚
  HW_GPIO_Init_Out(STEP_GPIO,X_STEP_GPIO_PIN);
  HW_GPIO_Init_Out(STEP_GPIO,Y_STEP_GPIO_PIN);
  HW_GPIO_Init_Out(STEP_GPIO,Z_STEP_GPIO_PIN);
  HW_GPIO_Init_Out(STEPPERS_DISABLE_GPIO,STEPPERS_DISABLE_GPIO_PIN);
  HW_GPIO_Init_Out(DIRECTION_GPIO,X_DIRECTION_GPIO_PIN);
  HW_GPIO_Init_Out(DIRECTION_GPIO,Y_DIRECTION_GPIO_PIN);
  HW_GPIO_Init_Out(DIRECTION_GPIO,Z_DIRECTION_GPIO_PIN);
...
void st_prep_buffer()
{
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    uint32_t cycles = (uint32_t)ceil( (TICKS_PER_MICROSECOND*1000000.0*60)*inv_rate ); // (cycles/step)
