...
uint8_t gc_execute_line(char *line)
{
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    int_value = (uint8_t)trunc(value);
    mantissa =  (uint16_t)round(100*(value - int_value)); // Compute mantissa for Gxx.x commands.
    ...
    switch(letter) {
      ...
      default:
        ...
        switch(letter){
          ...
          case 'N': word_bit = WORD_N; gc_block.values.n = (int32_t)trunc(value); break;
  ...
  */
  ...
  switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      coord_select = (uint8_t)trunc(gc_block.values.p); // Convert p value to int.
